...
use anyhow::Result;
use iroh_blobs::Hash;
use iroh_docs::{actor::SyncHandle, store, Author, NamespaceSecret};
...
async fn insert_n(
    ...
    for i in 0..n {
        ...
        sync.insert_local(ns, author, key, hash, 8).await.unwrap();
    }
}
...
async fn variant1a_stall_resolves_when_subscriber_catches_up() -> Result<()> {
    ...
    assert!(
        res.is_ok(),
        "insert should have completed after subscriber caught up"
    );
    sync.shutdown().await?;
    Ok(())
}
...
async fn control_set_succeeds_when_subscriber_is_drained() -> Result<()> {
    ...
    tokio::spawn(async move { while sub_rx.recv().await.is_ok() {} });
